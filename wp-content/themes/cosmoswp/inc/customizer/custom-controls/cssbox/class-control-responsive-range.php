 ...
 */
class CosmosWP_Custom_Control_Responsive_Range extends WP_Customize_Control {
	...
	protected function content_template() {
		...
		<div class="responsive-range {{wrapClass}}">
			...
			<input
					type="hidden"
					class="responsive-range-collector"
					title="{{{data.label}}}"
					value="{{data.value}}"
					{{{data.link}}}
			>
	...
	private function render_input() {
		...
		<div class="{{mediaQuery}} control-wrap {{active}}">
			<input
					class="responsive-range--input <# if( ! data.units ){ #>no-units<# } #>"
					type="number"
			<# if( attr.min ) {	#>
			min="{{attr.min}}"
			<# }
			...
			#>
			<select class="responsive-range--select" {{disabled}} data-default="{{attr.default_unit}}">
