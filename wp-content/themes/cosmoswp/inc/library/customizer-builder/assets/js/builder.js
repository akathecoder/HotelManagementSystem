(function( $, wp_customize ) {
    ...
    var is_rtl = CosmosWP_Customizer_Builder.is_rtl;
    ...
    var CustomizeBuilder = function( options, id ){
        ...
        var Builder = {
            ...
            container: null,
            ready: false,
            devices: {'desktop': CosmosWP_Customizer_Builder.desktop_label, 'mobile': CosmosWP_Customizer_Builder.mobile_tablet_label },
            activePanel: 'desktop',
            panels: {},
            activeRow: 'main',
            draggingItem: null,
            ...
            drag_drop: function(){
                ...
                $( '.cosmoswp-device-panel', that.container ).each( function(){
                    var panel = $( this );
                    var device = panel.data( 'device' );
                    var sortable_ids= [];
                    that.panels[ device ] = {};
                    $( '.cosmoswp-cb-items', panel ).each( function( index ){
                    ...
                    var sidebar = $( '#_sid_all-sidebar', panel ),
                        sidebar_id = sidebar.attr( 'id' ) || false;

                    $( '.cosmoswp-available-items .grid-stack-item', panel ).draggable({
                        revert: 'invalid',
                        connectToSortable: ( sidebar_id ) ? '#'+sidebar_id : false,
                        start: function( event, ui ){
                            $( 'body' ).addClass( 'builder-item-moving' );
                            $( '.cosmoswp-cb-items', panel ).css( 'z-index', '' );
                            ui.helper.parent().css( 'z-index', 9999 );
                            ui.helper.closest('.cosmoswp-available-items').addClass( 'cwp-available-item-moving' );
                        },
                        stop: function(  event, ui ){
                            $( 'body' ).removeClass( 'builder-item-moving' );
                            $( '.cosmoswp-cb-items', panel ).css( 'z-index', '' );
                            $( '.cosmoswp-available-items', panel ).removeClass( 'cwp-available-item-moving' );
                    ...
                    $( '.cosmoswp-available-items .grid-stack-item', panel ).resizable({
                        handles: 'w, e',
                        stop: function( event, ui ){
            ...
            addNewWidget: function ( $item, row ) {
                ...
                var that = this;
                var panel = that.container.find('.cosmoswp-device-panel.cosmoswp-panel-'+that.activePanel );
                var el = row;
                if ( ! _.isObject( el ) ) {
                    el =  panel.find( '.cosmoswp-cb-items' ).first();
                }
            ...
            addPanel: function( device ){
                ...
                var template_id =  'tmpl-cosmoswp-cb-panel';
                ...
                return '<div class="cosmoswp-device-panel cosmoswp-vertical-panel cosmoswp-panel-'+device+'" data-device="'+device+'">'+html+'</div>';
            },
            addDevicePanels: function(){
                var that = this;
                _.each( that.devices, function( device_name, device ) {
                    var panelHTML = that.addPanel( device );
                    $( '.cosmoswp-cb-devices-switcher', that.container ).append( '<a href="#" class="switch-to switch-to-'+device+'" data-device="'+device+'">'+device_name+'</a>' );
                    $( '.cosmoswp-cb-body', that.container ).append( panelHTML );
                } );
            },
            addItem: function( node ){
                var that = this;
                var template = that.getTemplate();
                var template_id =  'tmpl-cosmoswp-cb-item';
                if (  $( '#'+template_id ).length == 0 ) {
                    return ;
                }
            ...
            addAvailableItems: function(){
                ...
                _.each( that.devices, function(device_name, device ){
                    var $itemWrapper = $( '<div class="cosmoswp-available-items" data-device="'+device+'"></div>' );
                    $( '.cosmoswp-panel-'+device, that.container ).append( $itemWrapper );
                    $itemWrapper.append( '<div class="cwp-close-item-wrap"><span class="cwp-close-item-panel dashicons dashicons-no"></span></div>' );
            ...
            switchToDevice: function( device, toggle_button ){
                ...
                if( numberDevices > 1 ) {
                    $('.cosmoswp-cb-devices-switcher a', that.container).removeClass('cosmoswp-tab-active');
                    $('.cosmoswp-cb-devices-switcher .switch-to-' + device, that.container).addClass('cosmoswp-tab-active');
                    $('.cosmoswp-device-panel', that.container).addClass('cosmoswp-panel-hide');
                    $('.cosmoswp-device-panel.cosmoswp-panel-' + device, that.container).removeClass('cosmoswp-panel-hide');
                    that.activePanel = device;
                } else {
                    $('.cosmoswp-cb-devices-switcher a', that.container).addClass('cosmoswp-tab-active');
                }
   ...
   /*                 else if ( device == 'all' ) {
                        wp_customize.section( 'cosmoswp-menu-icon-sidebar-section' ).focus();
                    }*/
                }
                    ...
                    _.each( device_data, function( items, row_id ) {
                        if( ! _.isUndefined( items ) ) {
                            _.each( items, function (node, index) {
                                var item = $('.cosmoswp-available-items[data-device="' + device + '"] .grid-stack-item[data-id="' + node.id + '"]').first();
            ...
            focus: function(){
                this.container.on( 'click', '.cosmoswp-cb-item-setting, .cosmoswp-cb-item-name, .item-tooltip', function( e ) {
                    e.preventDefault();
                ...
                // Focus rows
                this.container.on( 'click', '.cosmoswp-cb-row-settings', function( e ){
                    e.preventDefault();
            ...
            remove: function(){
                var that = this;
                $document.on( 'click', '.cosmoswp-device-panel .cosmoswp-cb-item-remove', function ( e ) {
                    e.preventDefault();
                    var item = $( this ).closest('.grid-stack-item');
                    var panel = item.closest( '.cosmoswp-device-panel' );
                    item.attr( 'data-gs-width', 1 );
                    item.attr( 'data-gs-x', 0 );
                    item.removeAttr( 'style' );
                    $( '.cosmoswp-available-items', panel ).append( item );
                    that.updateAllGrids();
                    that.save();
                } );
            ...
            showPanel: function(){
                var that = this;
                this.container.removeClass('cosmoswp-builder--hide').addClass( 'cosmoswp-builder-show' );
                setTimeout( function(){
            ...
            hidePanel: function(){
                this.container.removeClass( 'cosmoswp-builder-show' );
                cwp_hide_item_panel(this.container.find('.cosmoswp-available-items'));
                $( '#customize-preview' ).removeClass( 'cb--preview-panel-show' ).removeAttr('style');
            },
            togglePanel: function(){
                var that = this;
                wp_customize.state( 'expandedPanel' ).bind( function( paneVisible ) {
                    if ( wp_customize.panel( options.panel ).expanded() ) {
                        $document.trigger( 'cosmoswp_panel_builder_open', [ options.panel ] );
                ...
                that.container.on( 'click', '.cosmoswp-panel-close', function(e){
                    e.preventDefault();
                    that.container.toggleClass( 'cosmoswp-builder--hide' );
                    if( that.container.hasClass('cosmoswp-builder--hide') ) {
            ...
            panelLayoutCSS: function(){
                //wp_customize.state( 'paneVisible' ).get()
                var sidebarWidth = $( '#customize-controls' ).width();
                if ( ! wp_customize.state( 'paneVisible' ).get() ) {
                    sidebarWidth = 0;
                }
                if ( is_rtl ) {
                    this.container.find( '.cosmoswp-cb-inner' ).css( {'margin-right': sidebarWidth } );
                } else {
                    this.container.find( '.cosmoswp-cb-inner' ).css( {'margin-left': sidebarWidth } );
                }
            ...
            init: function( controlId, items, devices ){
                ...
                var template = that.getTemplate();
                var template_id =  'tmpl-cosmoswp-builder-panel';
                ...
                if ( options.section ) {
                    wp_customize.section( options.section ).container.addClass( 'cosmoswp-hide' );
                }
                ...
                wp_customize.previewedDevice.bind( function( newDevice ) {
                ...
                wp_customize.state( 'paneVisible' ).bind( function(){
                    that.panelLayoutCSS();
                } );
                ...
                // Switch panel
                that.container.on( 'click', '.cosmoswp-cb-devices-switcher a.switch-to', function(e){
                    e.preventDefault();
                    var device = $( this ).data('device');
                    that.switchToDevice( device );
                    $('.cosmoswp-cb-body').find('.cosmoswp-available-items').each(function () {
                        cwp_hide_item_panel($(this));
                    })
                } );

                $document.trigger( 'cosmoswp_builder_panel_loaded', [ id, that ] );
    ...
    wp_customize.bind( 'ready', function( e, b ) {
        _.each( CosmosWP_Customizer_Builder.builders, function( opts, id ){
            new CustomizeBuilder( opts, id );
        } );

        wp_customize.bind( 'pane-contents-reflowed', function(){
            setTimeout( function(){
                if ( $( '#sub-accordion-panel-widgets .no-widget-areas-rendered-notice .footer_moved_widgets_text' ).length ) {

                }
                else {
                    $( '#sub-accordion-panel-widgets .no-widget-areas-rendered-notice' ).append('<p class="footer_moved_widgets_text">'+CosmosWP_Customizer_Builder.footer_moved_widgets_text+'</p>');
                }
        ...
        // When focus section
        wp_customize.state( 'expandedSection' ).bind( function( section ) {
            $( '.cosmoswp-device-panel .grid-stack-item' ).removeClass( 'item-active' );
            $( '.cosmoswp-cb-row' ).removeClass('row-active');
            if ( section ) {
                $( '.cosmoswp-cb-row[data-id="'+section.id+'"]' ).addClass('row-active');
                $( '.cosmoswp-device-panel .grid-stack-item.for-s-'+section.id ).addClass( 'item-active' );
            }
        });
         ...
         */
        /*
        wp_customize.state( 'paneVisible' ).bind( function( paneVisible ) {
            console.log( 'paneVisible state', paneVisible );
        });

        wp_customize.state( 'expandedPanel' ).bind( function( paneVisible ) {
            console.log( 'expandedPanel state', paneVisible );
            console.log( 'expandedPanel state Builder', wp_customize.state( 'expandedPanel' ).get() );
        });
        */
    ...
    $document.on( 'mouseover', '.cosmoswp-cb-row .grid-stack-item', function( e ) {
        var item = $( this );
        var nameW = $( '.cosmoswp-cb-item-remove',item ).outerWidth() + $( '.cosmoswp-cb-item-setting',item ).outerWidth();
    ...
    $document.on( 'mouseleave', '.cosmoswp-cb-row .grid-stack-item', function( e ) {
        $( this ).removeClass('show-tooltip');
    });

    /*Add a Item*/
    $document.on( 'click', '.cwp-add-new-item', function( e ) {
        var this_add_new = $(this),
        this_item_wrap = this_add_new.next('.cosmoswp-available-items');
        if( !this_item_wrap.length ){
            this_item_wrap = this_add_new.closest('.cosmoswp-cp-sidebar').next('.cosmoswp-available-items');
        }
        if( this_item_wrap.length ){
            this_item_wrap.toggleClass('cwp-show-items');
            $( 'body' ).toggleClass( 'cwp-body-overlay' );
            this_add_new.toggleClass('cwp-hide-items');
        }

    });
    function cwp_hide_item_panel(this_item_wrap){

        var this_add_new = this_item_wrap.prev('.cwp-add-new-item');
        if( !this_add_new.length ){
            this_add_new = this_add_new.prev('.cosmoswp-cp-sidebar').find('.cwp-add-new-item');
        }
        if( this_item_wrap.length ){
            this_item_wrap.removeClass('cwp-show-items');
            $( 'body' ).removeClass( 'cwp-body-overlay' );
            this_add_new.removeClass('cwp-hide-items');
        }

    }
     $document.on( 'click', '.cwp-close-item-panel', function( e ) {
        var this_close_item_panel = $(this),
        this_item_wrap = this_close_item_panel.closest('.cosmoswp-available-items');
